...
const fetchAndStoreProductsData = async (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const { title, images, stock, categories, types, mostSale, price } = await callApi(`https://api.ardunic.com/v1/product/${data[i]}`);
            ...
            if (!check) {
                ...
                if (categoriesFetchFilterList.length > 0) {
                    categoriesFetchFilterList.forEach(filter => {
                        // unFilteredProducts.push({ title, images, stock, categories: categories.map(c => (c.sections)).flat(1), mostSale, price: price.showPrice })
                        ...
                        if (checkCategory) {
                            ...
                            products.push({ title, images, stock, categories: categories.map(c => (c.sections)).flat(1), mostSale, price: price.showPrice, types })
                    ...
                    products.push({ title, images, stock, categories: categories.map(c => (c.sections)).flat(1), mostSale, price: price.showPrice });
                    createProductCard({ title, images, stock, categories, mostSale, price: price.showPrice });
                    total += 1
                    totalProducts.textContent = `Total (${total} / ${limt})`
                }
    ...
    if (categoriesFetchFilterList.length > 0 && totalFilterAmount < limt) {
        for (let i = 0; i < data.length; i++) {
            ...
            const { title, images, stock, categories, types, mostSale, price } = await callApi(`https://api.ardunic.com/v1/product/${data[i]}`);
            if (stock > 1 || types.length > 1) {
                const isPosted = postedProducts.length > 0 ? postedProducts.some(p => p === title) : false;
                const checkDub = products.some(p => p.title === title);
                if (!isPosted) {
                    if (!checkDub) {
                        products.push({ title, images, stock, categories: categories.map(c => (c.sections)).flat(1), mostSale, price: price.showPrice, types })
                        createProductCard({ title, images, stock, categories, mostSale, price: price.showPrice });
