...
let limt = Number(JSON.parse(localStorage.getItem("limt"))) || 80;
...
const render = async () => {
    ...
    limtInput.placeholder = `Default limt ${limt}`;
...
const fetchAndStoreProductsData = async (data) => {
    total = 0;
    let totalFilterAmount = categoriesFetchFilterList.reduce((prev, current) => prev + Number(current.amount), 0);
    totalFilterAmount = totalFilterAmount > limt ? limt : totalFilterAmount;
    for (let i = 0; i < data.length; i++) {
        ...
        if (stock > 1 || types.length > 1) {
            ...
            //CHECK IF THE PRODUCT IS POSTED
...
const createCategoryFilter = categoryName => {
    ...
    for (let i = 0; i < products.length; i++) {
        ...
        console.log(products[i].title === "MICS-5524 Gas Sensor");
        for (let x = 0; x < categories.length; x++) {
            const category = categories[x];
            if (categoriesFilterList.some(c => c === category.name)) {
                if (!filterdProducts.some(p => p.title === title)) filterdProducts.push(products[i]);
            }
        }
    };
    ...
    if (categoriesFilterList.length < 1) {
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            for (let x = 0; x < categories.length; x++) {
                const category = categories[x];
                if (categoriesFilterList.some(c => c === category.name)) {
                    if (!filterdProducts.some(p => p.title === title)) {
                        total += 1
...
limtInput.onkeyup = e => {
    ...
    limt = Number(e.target.value)
    setTimeout(() => {
        e.target.value = "";
        e.target.placeholder = `Default limt ${limt}`;
    }, 2000)
}
